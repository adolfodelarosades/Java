...
Comenzamos un nuevo bloque de vídeo nuevo capítulo sobre manipulación y tratamiento de datos y lo primero que vamos a hacer es conocer un poco más cama y es que habíamos dicho antes que estaba aparte de un lenguaje de programación y una plataforma sobre la cual ejecutar nuestro programa era una inmensa ingente cantidad de código puesta nuestra disposición para tratar un montón de situaciones ya conocidas es decir Java poner nuestra disposición un montón de clase en particular incluyendo más de 4000 clases vale en su versión 8 vale la versión de Java S4 mezcla se las podemos utilizar con total tranquilidad vale que requeriría un gran esfuerzo por rescatar parte el que nos sentáramos y estudiáramos para qué sirve cada una de estas cuatro mi clase viéramos cuáles son los métodos de cada una de ella cuáles son sus atributos y son públicos y son privados bueno no desesperemos porque Java pone también a nuestra disposición una documentación conocida como el API javadocs o bueno el abuso del lenguaje hace que se le llame directamente API vale y cada versión de Java tiene publicada esta ayuda este sábado vale no tiene publicado en la web lo podemos consultar entre particulares de la versión 8 no podemos consultar a través de este enlace que podemos visitar la web la primera se nos presenta una pantalla dividida en diferentes friends vale en el que podemos ver una lista con todos y cada uno de los paquetes que nos aconsejaban listado de todas las clases las 4000 y pico que tiene independientemente del paquete en el que estén una presentación en este caso más específica de los pintos paquetes que tiene y cuál es el cometido de cada uno de ellos por ejemplo el paquete de Java SQL probé del lápiz necesario de los métodos necesarios de las clases necesarias para acceder y procesar datos almacenados en un origen de datos normalmente una base de datos relacional usando Java o por ejemplo la java.nio nos permite manejar contenedores de datos va a hacer vale y y una manera nueva de acceder a fichero y otros orígenes de datos el paquete más no ofrece clases matemáticas si pinchamos en un paquete nos llevará directamente al contenido de ese paquete vale y dentro de él podemos ver el sumario de clase y de numeraciones que no son más que un conjunto cerrado de valores podemos encontrar una descripción del paquete donde nos hablará pues algo sobre las diferentes clases y ya están dentro del paquete pinchar amo en el nombre de una clase nos llevará a la información sobre esa clase si quería comentaros que podemos eliminar estos prende la izquierda sino resultan incómodos dándole a la versión de nou sense vale dónde es bueno pues ya nos mostraría directamente solo la información del paquete Java para qué sirve la clase vikinger por ejemplo ya no está aquí una primera descripción donde nos dice que es una clase inmutable es decir que cuando construyamos un objeto no va a ser no va a poder ser modificado y no sé lo que hace que podemos tener otro para guardar enteros de revisión de precisión arbitraria el destino entero todo lo grande que nosotros queramos todo lo que te debo capítulos gratis .

Más extenso y menos pensó nos dice por ejemplo que un valor integral puede estar sobre todo en este rango siempre que sea positivo no si es suficientemente grande y más adelante nos indica los atributos que tiene los constructores y un resumen de los métodos para cada uno de ellos incluso podríamos pulsar lo dice bueno la clase distinta que eres particular tiene aquí una serie de campos en este caso son un poco especiales porque son de su mismo tipo y no indican determinados valores concreto y el uno entero y el 10 valores constantes Vicente los coches stores los distintos constructores a partir de un valor en una sucesión de byte a partir de una representación en una cadena de caracteres la cantidad de metros no podríamos recordar de memoria ya no de una sola clase si no más de una clase sería imposible que recordáramos todos los métodos no hace falta porque los ponemos consultar aquí si queremos saber en particular lo que hace un método por ejemplo el método han nos lleva una descripción donde nos diría la firma del método es decir el tipo de dato de retorno el modificador de acceso vamos a darle un poquito mejor el tipo de dato de retorno valer modificador de acceso el nombre del método los argumentos que reciben para qué sirve la identificación de los distintos parámetros lo que devuelve el método aquí tenemos los diferentes métodos para poder verlo vale dime una descripción totalmente exhaustiva de lo que realiza vale como por ejemplo el método isprobableprime que devuelve un método que es probablemente un número primo con un grado de certeza y que aquí como trabajar con este método no vale y otra serie de métodos vale la representación cuestión etcétera igual que tenemos esta clase ya digo que tenemos un montón de paquetes nuevos perdón de de paquetes diferentes para usar por ejemplo hemos usado la clase String que se encargaban hay para que te avalan este tiene varios instanciar una clase C de cómo utilizar la una explicación de algunas clases que tienen asociación con ella los constructores una serie de método etcétera etcétera cómo podemos comprobar sistema de ayuda se vuelve indispensable para nosotros si queremos poco a poco irá aprendiendo toda la variedad de clase de paquetes que estaban ofrece y los métodos que utiliza como decía antes esta ayuda que tenemos por aquí en la que clip se nos muestra ante toda esta ayuda en la que no aparece cuando nosotros situamos la ratona que encima lo único que hace es consultar vale pues no lo mostraría LAN la clase interna tenemos por aquí el método en el ejemplo que es Shein caracteres tendríamos una descripción de lo que hace el método de los parámetros que recibe de su valor de retorno si es posible que lance algún tipo de error toda esta información la atención de esta manera iremos conociendo poco a poco todas las clases refrescaba y todas sus posibles
