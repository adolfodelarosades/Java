...
Continuar con el siguiente bloque perdón dentro de este mismo bloque con el siguiente vídeo en el que vamos a hablar de Array creación y uso de array todos los tipos de datos que hemos conocido todas las variables incluyan siempre almacenar un valor concreto en el caso de los entero un número entero los números reales doble por ejemplo pues el valor civil en el caso de un carácter una letra suelta una cadena palabra mensaje sin embargo hasta ahora no hemos podido almacenar un conjunto de valores de un tipo primitivo estructurado de manera homogénea y juntas sino que lo hemos tenido que hacer a través del referente cuenta cuando hemos querido tener tres personas diferentes hemos tenido que crear tres referencias y citas sin embargo veremos como conforme vayamos construyendo programas cada vez más complejos como senos no hace necesario el tener un contenedor de elementos de un mismo tipo array no está respuesta a este problema dándonos un contenedor que tendrá un tamaño fijo cuya longitud se establece en el momento de crearlo y que nos permitirá albergar diferente elemento a cada posición de Array me llamarán elemento vale eran del mismo tipo como elemento primordial para conocer puede que suene a partir una pera importante que lo tengamos presente que los arrays comienzan a contar en 0 informáticos son típicamente conocidos porque comienzan a contar en celo las cosas vale el valor 0 es muy importante entonces una raíz que tenga longitud pie se tire capacidad para 10 elementos tendrá como índice de cada posición es decir el número asignado a cada posición para la primera posición cero para la segunda 1 parada emperador y así sucesivamente para la décima posición su índice será el nuez como vemos en la imagen elemento en el índice 8 es perfecta en la novena posición cómo podemos crear un array más ligado a la creación de un objeto que a la de la de un tipo de dato primitivo ya que tenemos que hacer uso de la palabra añil debemos indicar primero el tipo de dato y el tamaño pero para hacer explícito que estamos creando un array añadimos dos [una apertura y cierre de corchete al tipo de datos tenemos que tener presente que el tipo de dato del array que estamos declarando a la izquierda del operador de asignación tenemos que usarlo después para indicar el tamaño nada pero vayamos de izquierda después del tipo de datos como notición digamos lo corté el nombre del array será como el nombre de cualquier otra variable operador de asignación operador distanciacion y a continuación como decía volvemos a indicar el tipo de dato y entre corchetes lo que indicamos se llaman de la RAE como decía ese tamaño es disco de inicia por si me interesa mejor dicho nada eh con ese tamaño el tamaño no se puede variar no quiere decir que la GRAITEC adentro todo eso colores todavía no luego vamos aprender ahora como cómo hacerlo si queremos acceder a una
