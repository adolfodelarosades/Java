...
, hola a todos en este vídeo vamos a hablar de modificadores de acceso a nivel de clase pero sobre todo a nivel de atributos y de meterme recordemos que Java nos proporcionaba cuatro tipos diferentes de modificadores de acceso público privado protegido y el llamado por defecto que era equivalente a no poner ningún modificada en esta tabla podemos ver dentro de los diferentes niveles de agrupamiento de código una clase un paquete una subclase todavía no lo conocemos pero es buen momento para que hablemos de modificadores de acceso una subclase será una clase que heredé de otra clase en el próximo capítulo en el próximo bloque hablaremos sobre sobre herencia y el mundo en general es decir cualquier otra clase que estuviera en otro paquete diferente no dentro de esos cuatro ámbitos podemos definir o utilizar alguno de los diferentes modificadores de acceso me cago en los distintos colores valen los que si se puede acceder y no una clase un atributo o un método que esté declarado con modificadores Republic podrá ser accedido desde cualquier sitio vale cualquier cualquier método por ejemplo de esta clase podrá acceder a él si fuera por ejemplo un atributo cualquier método de cualquier clase de su mismo paquete también podría una clase que heredará de él también podría y cualquier otro método de otra clase que estuviera en otro paquete diferente escribir podría podrías tener en el caso de los atributos por ejemplo con métodos que tuviéramos como protegidos con ofrecerte vale podrían exceder los mismos que antes pero solamente descartamos a cualquier otra clase que estuviera fuera del paquete donde hemos decidido las plantas decir que el modificador protegido permitiría que cualquier sobre todo bien orientado para herencia no que cualquier clase que heredará de la nuestra pudieran generar pero no solamente eso sino cualquiera método de otra clase de nuestro paquete también podría podría acceder a el sobre

Pero no solamente eso sino cualquiera método de otra clase de nuestro paquete también podría podría acceder a el hablar de manera más profunda sobre lo sobre el modificador protected cuando cuando hablemos de en los primeros ejemplos no utilizamos ningún tipo de modificador bueno una clase comiendo va a poder ser accedido desde la propia clase en tanto en cuanto ponemos utilizamos el modificador privado solamente la propia clase podrá acceder a ese método ese o ese atributo como podemos comprobar los modificadores de acceso nos pueden ayudar a modular el nivel de encapsulación que queremos aplicar a diferente atributos método o incluso classic no hablemos individualmente ahora de clase de método y de atributos hablando de clase la mayoría de las clases por las que nosotros vayamos a trabajar la mayoría de las clases que vayamos a implementar siempre serán públicas no por lo menos con lo que sabemos ahora o incluso pues serán clase que no tenga modificado pero ya digo sobre todo serán clases públicas de serlo cualquier persona podría utilizar la selección nosotros dentro de nuestro programa o si estamos programando otro tipo de código orientado a que sea una librería de soporte o haciendo código de soporte para para limpiar a otro tipo de proyectos por cualquiera podría utilizar las claras si no le ponemos modificador solamente vamos que podíamos utilizarlo con clases del mismo paquete no ya digo lo normal es que trabajemos con clases y sean que sean públicas sin embargo al hablar de métodos empezamos a poder matizar si bien la mayoría de los métodos de una clase son públicos entre el público recordemos la imagen del iceberg en la que bueno la encapsulación no decía que los objetos interaccionan entre ellos mediante paso de mensajes decir mediante invocaciones a método siempre que tenemos que plantear no escoger siempre la versión más restrictiva es decir tenemos que ser muy celosos a la hora de hacer la encapsulación la mayoría de los métodos eran eran publi incluido los constructores los constructores van a hacer en el 99% de las ocasiones amanecer público pero es verdad que podemos plantearnos la posibilidad de utilizar o redefinir metodos que harán operaciones auxiliares intermedia es que nos podría interesar marcarlos como como privado algún ejemplo podéis encontrar en código que hemos visto como en el en el juego de hundir la flota en el que poníamos algunos metros que no se iban a trabajar directamente o que no se permiten invocar a porter and operaciones auxiliares y estaban marcados como como privado como digo siempre intentaremos escoger la versión más restrictiva posible


A nivel de atributo sucede lo contrario que en la gran mayoría de las ocasiones los atributos serán privados la encapsulación salgo para las constantes que bueno que usualmente las constantes para poder utilizarlas no solamente dentro de nuestra clase sino como comparación fuera de ella las podíamos mandar como no públicas no hay que tener una muy buena razón para no usar private como el modificador de acceso de un atributo no ya que bueno el uso de atributos público sino acceder a ellos a través de métodos hace que el código sea usted muy acoplado y por tanto limita la posibilidad de que nosotros modifiquemos ese código en el futuro lo que se conoce como la refactorización no necesito irme no fue poder modificar como como ese código trabaja por dentro sí que hacen que afecte a a la gente que lo utiliza no porque si cambiáramos el tipo de dato de variable o si cambiáramos el nombre de esta variable dentro del objeto todo código cambiamos el nombre pero siempre hacemos ella a través del método tendríamos que modificar solamente el cuerpo de son método 1 pero tiene lo utilizara pues no tendría que modificar su código no con lo cual mi recomendación es que siempre trabajemos con atributo en la medida de lo posible privados ya digo algo que tengamos una muy buena razón para trabajar con atributos público vamos a ver un ejemplo de varias clases con diferentes tipos de modificadores de clase que estén en el mismo paquete que estén en diferentes paquetes y como podrían interaccionar supongamos que tenemos este código como podemos ver tenemos tres paquetes diferentes uno llamado paquete C otro llamado modificadores y otro paquete que tiene dentro un paquete paquete recordemos que teníamos la posibilidad de usar público privado protegido y por defecto como no vamos a tener herencia en ningún caso vamos a usar protegido vale vamos a usar público privado y por defecto vale comencemos por la clase a la clase A está definida en el paquete a qué es un su paquete de modificadores su modificador de Acceso Este por defecto es decir que todas las clases del mismo paquete podrían acceder a esta clase A y además un método y su propiedad también con modificador por defecto es decir cualquier clase de ese paquete podría acceder a ellas no si intentamos desde una clase que está ubicada en otro paquete aunque sean paquetes que estén relacionados no paquete a es un SUV paquete de modificadores si intentamos acceder a ella desde fuera de su paquete directamente ya eclipse no indica que no es posible acceder porque no está definida dentro del mismo paquete si creáramos otra clase nueva paquete y quisiéramos crear una nueva instancia de a como propiedad de dentro del mismo paquete dentro del mismo paquete
