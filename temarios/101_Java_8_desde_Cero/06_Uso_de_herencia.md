...
Vamos a presentar las clases marcadas como Astra estas clases abstractas son clases de las cuales no se pueden crear instancia S7 van a servir de alguna manera como plantilla para que una clase tienda de ella pero no podríamos tener distancia de ellas por eso se llama abstracta pueden tener métodos que tengan implementación pueden tener métodos también abstractos un método abstracto son método de los cuales pues decimos que estaréis a decir que esta clase tiene este método que cualquier clase que heredé el debe darle cuerpo es decir que funcionan como como los métodos que veíamos podemos llamar normales de los que nos comprometimos a implementar una clase abstracta también puede tener método marcados como abstractos no entonces las clases que tienda a esta clase abstracta tendrá que implementar si no los quisiera implementar esa clase que heredé de esta primera clase abstracta también tendría que ser marcada como abstracta y métodos abstractos y métodos no abstractos pueden convivir de manera normal no vamos a ver alguno alguno ejemplo clase hasta primero podemos comprobar como tenemos una clase abstracta que hemos llamado sencilla con un solo método para saludar y que bueno Lali hay clase con la mundo una clase aita abstracta sencilla que no tiene más implementación se refiere que solamente va en el ejemplo podemos comprobar si intentamos crear una instancia de la clase abstracta sencilla nos dice que es imposible porque porque está definida como como hace no si podríamos quedar distanciate derivada y al igual que sucede con cualquier otro tipo de herencia de clases podríamos usar abstracta sencilla como el tipo de referencia en el que vamos a guardar está distancia no sé si llamamos al método saluda podríamos comprobar como te está llamando a la implementación de la clase abstracta del método saluda en este otro ejemplo tenemos una clase abstracta que al igual que antes tiene un metro con implementación pero también tiene un método abstracto qué es el método saludo y que recibe un argumento la clase que derive de abstracta y no queramos que a su vez sea abstract tiene que dar implementaciones de metro de hecho que esto lo comentamos podemos comprobar como al igual que sucedía con la interfase eclipse ya se canse y anticipando al error de compilación y no está sufriendo que tenemos que implementar esos métodos abstractos que tiene la clase de la cual estamos heredad vale con lo cual nosotros le daríamos le daríamos cuerpo si abstracta con métodos tuviera más de aquí lo tenemos más de un método abstracto veríamos como a la hora de implementar decir se los pones son no solamente un método sino que nos pondría dos métodos vale tantos como métodos abstractos tenga la clase de la cual estamos estamos heredando el ejemplo si creamos la clase derivada en este caso fue creado con la referencia de derivada veríamos no podemos llamar a saludar que proviene de la clase abstracta y a saludo bueno que esté en la referencia concreta de la clase derivada no esta manera podemos ver cómo utilizar las clases abstractas como base para que podamos poner el código que iba a ser común a diferente a diferentes implementaciones de otras clases más concretas y que podríamos tener otros métodos definidos con más trastos que le

Alguien se preguntará bueno en Java 8 la interfase también implementaciones por defecto ya podemos dar el código cuando tenemos que usar uno cuando tenemos que usar otro pues la respuesta más sencilla la respuesta como suele suceder en muchas ocasiones es que depende si es que es bueno tendremos que valorar tenemos que tener presentes que las diferencia y similitudes entre interfaz y clase abstracta para poder elegir la mejor opción café no se pueden instanciar como tales las clases abstractas tampoco lo interfaces tienen métodos en implementación y las clases extras también no interfaces tienen métodos con implementación por defecto al igual que las clases abstractas e interfaces pueden tener atributos solamente estático o constante frente a las clases abstractas que podrían tener como cualquier clase cualquier tipo de atributo el cual te daría las clases que tendieran a la clase abstracta e interfaz pueden tener métodos públicos o por defecto frente a las clases abstractas que pueden tener públicos privados protegido por defecto y la gran diferencia estriba en que una clase puede implementar multiple interfaces como ya hemos comprobado frente una clase que solamente puede heredar de ella por lo cual que deberíamos usar pues bien deberíamos usar interfaces cuando tengamos clases que no están relacionadas pueden implementar los métodos también deberíamos usar interfaces si se quiere indicar que existe un tipo de comportamiento como el relatable qué planeta vamos antes pero no sabemos quién va a ser el que lo implemente sí y el común denominador de estas clases para hacer un elemento de comportamiento sería más recomendable utilizar una intensa las clases abstractas feria más recomendada y queremos compartir el código con clase que está muy relacionada sobre todo a nivel estructural no pensábamos antes del uso de figura pues sería un gran ejemplo del uso de clase abstracta podríamos tener la clase base 6 figuras y podríamos tener triángulo rectángulo círculo porque fue no tendrían que entre elementos estructurales en común para para una figura también deberíamos usar clase abstracta y las clases derivadas van a intentar usar métodos protect estivales en private pues series más recomendadas sobre todo nos marcará la diferencia el hecho de que si tenemos una clase que ya extiende de otra clase no podríamos hacer que además se extendiera de una clase abstracta con lo cual en ese caso si necesitamos tener algo parecido a herencia múltiple usaremos la interfaz en el caso de que la clase que estamos definiendo no esté heredando de nadie de verdad que bueno pues sí sí lo que se comparte mal a nivel estructural podríamos tener una clase abstracta así de esa manera en la práctica podríamos definir atributos que no sean estático constantes y podríamos heredar con esto terminamos está este apartado de gerencia de polimorfismo de trabajar con interface y sobre todo con en este último apartado con las clases
