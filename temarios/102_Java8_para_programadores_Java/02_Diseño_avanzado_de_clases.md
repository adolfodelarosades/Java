...
Vamos a cerrar el bloque de diseño avanzado de clase hablando de la enumeración puede que sea una estructura que en ocasiones pase desapercibida o que Suso queden minusvalorado ha muerto frente a que se utilicen una serie de constante pero vamos a ver cómo cómo incluso son más potentes que no te lo lenguaje de programación y que bueno pues tienen cierta utilidad el tipo enumerado es un tipo de dato especial de utilizar la palabra reservada en 1 y en primera instancia no sirve para definir un conjunto cerrado de valores que nos valgan como comparación alguno ejemplo pues la dirección una dirección que podría ser Norte Sur Este Oeste y que nos valdría siempre y cuando lo utilizaremos para cenar nos valdría para comprar o por ejemplo los días de la semana no de de lunes a domingo en principio sería un uso sencillo de la enumeración no para comparar los días de la semana veamos un sencillo ejemplo antes de entrar en en otro más complejo aquí tendríamos la posibilidad de crear una enumeración con los días de la semana para crear una enumeración podemos utilizar el asistente que nos ofrece eclipse vale que es muy parecido al de crear una clase tan solo tendríamos que definir una serie de valores separados por coma y con ello tendríamos nuestra enumeración terminada podemos ver cómo podríamos plantear un método que recibiera un día de la semana y por numerada y utilizarlo para comparar vale lunes martes miércoles jueves viernes sábado vale por ejemplo si queremos ver según tengamos el cuerpo ese día para saludar los lunes son días realmente duro los viernes son bastante mejores los findes son los mejores días y el resto de días pues un científico y no bueno pues podríamos saludarte un día como veis proporcionamos un valor de tipo enumerado y utilizamos el tipo enumerado para comparar vale esa manera vamos a poder siempre comparar con un valor que sepamos que pertenece a la enumeración porque nosotros realmente no le estamos dando por detrás en ningún valor específico aunque estaba realmente si lo está haciendo no y aquí bueno pues sacáramos podemos ver cómo sería saludar un lunes miércoles viernes y un sábado sin embargo el uso de la de los tipos enumerados en Java como decías más potentes y para Java una enumeración en el fondo es una clase con lo cual cada uno de los elementos que definamos en la enumeración podría ser instancia de esa clase una enumeración puede incluir método puede incluir atributos de hecho cuando se compila y se ejecuta Java añade algunos métodos especiales como métodos válidos que nos permite obtener todos los valores de la enumeración y como decía podemos si antes no podíamos ver como un conjunto cerrado de valores que se utiliza para comparar lo podríamos ver cómo un conjunto cerrado de instancias de una clase que tienen método y que tiene la tienda podemos asimilarlo al a una serie de planetas no los planetas del sistema solar podríamos definir una enumeración con Mercurio Venus la Tierra hasta Neptuno y bueno este numeración como ya digo especial para tener atributos iba a tener método entre ellos constructores de cada planeta vamos a querer saber Dos Santos su masa en kilogramos y el radio del planeta en metros cómo podemos comprobar para indicar que vamos a tener dos atributos seguiríamos la estructura de una clase no en este caso si podemos identificar rápidamente que van a ser objetos inmutable no con lo cual bueno directamente ya podemos asignar los valores como final vale para que una vez que se haya construido el objeto no se puede modificar una vez que hemos tenido aquí el constructor podemos comprobar como a la hora de definir el conjunto cerrado de valores lo hacemos indicando el nombre de cada uno de los valores del tipo enumerado pero pasándole una serie de datos a su consulta vale un particular son datos reales de lo de la masa en kilogramos de cada uno de los planetas y de su radio tendríamos como es natural el método Keter que no se te ve vale que nos permitirían saber la masa y el radio de cada uno de los planetas podríamos definir también la constante universal G vale la constante gravitacional universal que nos puede servir para calcular la gravedad en la superficie de un planeta y también el peso en la superficie vale en función de la gravedad superficial es decir toda una enumeración bastante completa con algunos cálculos físicos de esta manera podríamos utilizando los planetas decir bueno si este es nuestro peso en la tierra podríamos calcular nuestra masa en la tierra que aquí en particular será la misma y recorrer con el método values todos los planetas con lo cual podemos iterar sobre el cómo podemos comprobar es que así como si estuviéramos trabajando en una instancia de una clase y podríamos indicar el peso en función de nuestra masa vale en la superficie de los diferentes planetas cómo podemos comprobar nuestro peso en la tierra calculando la masa es el mismo que el peso que debemos de seguir de partida pero en los diferentes planetas en función de la constante es que el radio de la gravedad de la atracción pues podríamos calcularlo con este tipo enumerar esto nos permitiría crear nueva instancia qué planeta es decir si se volviera a considerar Plutón como una instance perdón como planeta del sistema solar no podríamos crear una nueva instancia tal cual se nos tendríamos que modificar el tipo enumerado para añadirlo allí dentro con esto terminamos este vídeo de numeraciones terminamos el bloque de diseño de clase y nos lanzamos de lleno al siguiente
