...
Hibernate como ORM nos va a aportar una serie de características que son muy positivas, el primero es la productividad, como hemos podido comprobar en el capítulo anterior escribir código para acceder a una base de datos relacional desde un programa orientado a objeto en particular con Java y JDBC exige un gran rendimiento por parte del programador, sobre todo por que le va a dedicar mucho tiempo a determinadas tareas que no son primordiales como ese código de conexión a la base de datos, de lanzar la consulta y sobre todo de procesar la fila que nos hemos traído en un objeto de nuestro modelo.
...
queda queda incluido dentro de los rmsl que se conoce como el JP qué es la implementación del estándar de la tecnologíapues tendremos todas nuestras clases a utilizar necesario como la configuración pues va a ser bastante sencilla otro módulo que ya quedado también de una manera casi práctica integrada dentro del CRM eso que lo podemos utilizar por separado es el validator se trata de un módulo que implementa el estándar jsr-303 y que nos va a permitir validar pins de una manera muy sencilla de hecho con una sola boca anotaciones pues podremos podemos validar nuestro nuestros beans nuestra entidad es vale y todo eso se llegará a propagar a la base de datos y así lo creemos hibernate envers en una tecnología que hibernate y también incorpora dentro de de de su novio core la última persona si tiene una tecnología que ya tiene trayectoriainteresa persiguiendo no solamente la versión actual sino los diferentes cambios que han ido sucediendo a nivel de selecciones de modificaciones de una o varias columnas o de eliminación de datos es una tecnología bastante potente y que puede facilitar potingue está auditoría de cambio de información que puede ser más sensible y vernets es una implementación de un buscador FULLTEXT vale que nos va a permitir realizar búsquedas de texto completo a través de una base de datos de Apache que usted vale y nos permitiría poder realizar búsquedas más allá de las que permite la sentencia SQL o las comparaciones de igual o online para las bases de datos NoSQL la base de datos que van más allá del modelo relacional tenemos el mapeador objeto green vale y que nos va a permitir realizar lo mismo las mismas tareas que realizan reme pero con soporte hacia base de datos no es secuela también incorpora soporte para usarlo con el estándar jpg ahora vamos a comprar y verme con otro RM o productos que son parecido a RM del mercado si comenzamos con la especificación JDBC y como decíamos antes pues la más básica dejaba en la más bajo nivel para poder programar acceso a datos pues nos encontramos con que cada una tiene sus ventajas y sus detectar las ventajas que puede tener JDBC o más natural es un mayor rendimiento al añadir menos capas de abstracción que entre nuestra aplicación y el sistema gestor de base de datos concreto en particular ser más encontramos algún tipo de driver JDBC para no estar base de datos en particular que sea rápido pues el rendimiento de las consultas la velocidad de acceso puede ser bastante bastante mayor además como decíamos antes no puedes permitir utilizar funcionalidades concretas específicas del sistema gestor de base de datos que hemos escogido sin embargo ofrece unas ventajas que para mi gusto tienes bastante peso el primero el mayor tiempo en el desarrollo y la implementación de lo que se conocen como las clases dado también conocidas como las clases Cruz de se lleva muchísimo tiempo programar esa parte de los metros con la novia de conexión a la base de datos con rescatar con conocer la sentencia SQL concreta transformar una fila en objeto de una tabla manejar la asociación es algo que se lleva muchísimo tiempo por ende el mantenimiento de este software es mucho más costoso si desarrollamos una aplicación que tiene 108 miles de líneas de código para acceder a la base de datos cualquier cambio que queramos realizar cualquier devolución en nuestro software siempre requerirá además esfuerzo además el mayor número de líneas de código va a provocar que los errores durante el desarrollo simplemente con hibernate la ventaja que tenemos frente este desarrollo lento pesado con JDBC tenemos un desarrollo más nos vamos a centrar en lo has puesto concreto ya se encargará del resto nosotros nos centraremos en definir bien nuestro modelo de negocio nuestro servicio y hibernate hará el resto usaremos entidad es decir objetos algo que en lo que llevamos tiempo programando con orientación objeto conocemos y manejamos bien frente a tener que usar pues consultas tablas filas efecto y además vamos a eliminar el flores de ejecución al iniciarse y verme pues va a ser capaz de comprobar todo ese código de Mateo si lo hemos definido mediante descriptores NL hola anotaciones de Mateo y si existe algún error nos lo dará antes de iniciar la ejecución de la aplicación estoy con JDBC no pasaba y los errores las más veces sucedían en un momento concreto al pulsar un botón insertar un valor donde hemos tenido algún problema en una sentencia SQL no todo iba a ser ventaja el uso de Internet pues también va a suponer alguna que otra desventaja el primero es que la curva de aprendizaje más pronunciada que con JDBC JDBC tenemos si no queremos meter no hay mucha profundidad tenemos baile mentalmente 3 interfacesañadir zapater fracción el hecho de que se tenga muchos datos en memoria sino no sabemos manejar bien pues vas a provocar un menor rendimiento y hemos hablado antes que las cachés pueden facilitarnos el hecho de que nuestra aplicación no tenga que consultar de manera real la base de datos demasiada ocasión otra solución tampoco es que le das a conocer como 1RM en particular sino que no es un transformador a través de consultas de objetos también si bien en MyBatis hibernate la librería pero pero no permiten generar en nosotrosademás de eso pues muy escalable decir que nuestro sistema no puede ir creciendo con el tiempo con las Mercedes e ir escalando poco a poco manteniendo el nivel de rendimiento sobre todo muy avanzada utiliza directamentees fácil cambiar de tema MyBatis lo que haces mapear un resourced que un objeto que recoge los datos de una consulta a la base de datos mediante JDBCcomo ventaja de MyBatis sobre hiberne pues como nos permite acceder directamente a veo con ese cuerpo es mucho más fácil usar procedimiento almacenado o funciones que estén en el sistema efector frita y verme pues un poco más un poco más veamos ahora las diferencias y semejanzas de hibernate y JP porque bueno si investigamos por internet hay veces que la información puede llegar a ser confusa y no sabemos si son lo mismo si son cosas diferentes y son complementarias etcétera de una forma un poco poética podríamos decir que si j.p.a. es el baile hibernate el bailarín o si es el arte y verme es el artista es un estándar es decir no es más que un documento una especificación que surge para dar un poco de orden a surgir de hoy en Java que uso bastante años y forma parte del estándar de Java hibernate en cambio es aparte de 1RM un producto en sí mismo es también lo que se conoce como un horno una implementación de scandal jpga una base de datos relacional crema también muy sencillo para realizar la operación de acceso inserción modificación y borrado de datos nos da un un lenguaje de consulta y también una programático para para poder consultar la información almacenada en esa entidad de y nos dan múltiples elementos de optimización todo eso viene definido en el estándar JP a quien no ofrece un conjunto de funcionalidades que van mucho más allá de edificación Getxo quiénes desarrollaron escribieron esta explicación se basaron paraquién es marcada dentro de un conjunto mucho más amplio que sería divertido que yo ofrece una serie de funcionalidades específica pues que es mucho más grande una arquitectura típica en una aplicación sería aquella en la que sobre servlets jsp jsf enterprise JavaBeans etcéteradirectamente o tendríamos la posibilidad de utilizar con la implementación de Internet para acceder a nuestros a por último en este capítulo vamos a hablar un poco de hibernate email si si entrará Montseny en la página de hibernate y nos descargamos pues la totalidad desde lo RM vale que es un fichero zip que bueno que nos podemos descargar podríamos incorporar esos ficheros a nuestro proyecto nos daremos cuenta que solamente para para incorporar el cole tenemos que manejar viejas distintos además es hoja si lo miramos por dentro vemos que tienen más de 300 paquetes más de 6000 clases con lo cual bueno fue podríamos llegar a sufrir lo que se conoce como el infierno del Óscar no en el que al añadirlo uno podría pisarse sobre otro podríamos encontrar con quedó definen una misma librería podría hacer algo o terrible bien como en otros cursos invitamos a que el manejo de estas librería la hagamos a través de un sistema de gestión de dependencia particular nosotros estamos como estamos como sistemas por facebook no fichero de proyecto en el XML con tan solo añadir una dependencia como está pues todo eso ficheros cab todas esas librerías que vamos a necesitar quedarían incorporada en particular podemos acceder a a la última dependencia si entramos en la página de internet vale que está en hibernate.org en el módulo de CRM si accedemos a aparcar de descarga de doblo en la parte de abajo encontraremos siempre pues las últimas dependencias maven que están publicadas otra manera de buscarla también en mediante la navegación dentro del repositorio de maiden que como sabéis SD a través de la URL mvn repository puntocom y si utilizamos su buscador pues podríamos encontrar las diferentes las diferentes librerías que están publicadas así como pues código XML de la dependencia meiben o incluso de otros sistemas que tendríamos que incorporar a nuestro proyecto en la próxima lección el próximo capítulo vendrá María lecciones veremos cómo crear nuestro primer proyecto con hibernate con JP a y también lo haremos un poco con este
