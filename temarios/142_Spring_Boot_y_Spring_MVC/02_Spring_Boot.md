...
Vamos a seguir aprendiendo sobre spring boot y vamos a ver qué sucede con la estructura del código y algo que llama la atención porque frente a otros frameworks que nos obliga a que nuestro código sigue una estructura determinada spring boot nos da total libertad para poder estructurar el código de nuestro proyecto y esto es una gran facilidad porque podemos utilizar bueno pues que más de trabajo costumbres que tengamos dentro del equipo de desarrollo para no tener que desviarnos lo que sí podemos decir que deberíamos seguir una serie de buenas prácticas la primera es vale y esto suele suceder mucho con algunas personas que empiezan a trabajar con Spring o incluso con Java y que se quieren iniciar a la BNC el hecho de no colocar las clases en un paquete y que esté en el llamado paquete de folios vale el paquete por defecto que es donde estaba colocado una clase que no tienes paquete utilizamos spring boot y la anotacion spring boot application que bueno por defecto se ha usado el asistente de clinicia laisser pues ya no la coloca podemos encontrarnos con problemas si habéis pasado por nuestro curso de Spring core sabréis que a la hora de crear componentes Vince vale o algunos componentes derivado bueno si usamos la anotación es lo más interesante que queremos es que se nos autoescaner un determinado paquete con sus paquetes para localizar esos componentes y no tienes que registrarlo de ninguna manera si usamos la anotación de Spring boot application lo que hará por defecto Spencer a buscar la las clases que estén dentro del paquete donde esté esa clase o sus paquetes lo dejamos en el paquete default post seguramente ese esos componentes que hagamos no se localicen no se pueden encontrar por tanto es muy recomendable que utilicemos siempre una estructura de paquete además los paquetes se pensaron para que bueno pues fueran accesibles con una anotación a la inversa vale de un dominio por ejemplo si el dominio de nuestra empresa fuera openwebinars.net trabajar en un proyecto llamado Proyecto el paquete donde deberíamos colocar la clase notada con Spring boot application esa clase con el mensajería con punto openwebinars.net o siempre y ya veremos que lo haremos a lo largo de todo el curso ser habitual que tengamos una clase principal con el método main anotada con Spring boot application y ese será como digo el paquete que tengamos de raíz porque ya digo que delimitarán lo que se escanea automáticamente todo lo que coloquemos deberá estar siempre en ese mismo paquete o en paquetes que estén dentro de ese paquete por ejemplo aquí tenemos una estructura que nos permitiría organizar el código vale un supuesto código de una aplicación en diferentes en diferentes paquetes vale como decía si este paquete con example of my application es donde está la clase application Java y esta es la que tiene anotada spring boot application la que tiene esta anotación si creamos otro paquete que sea por ejemplo con example that habéis y añadimos ahí algún bean ese vinos escanear automáticamente tendríamos que proporcionar nosotros los elementos de configuración para que freírlo localizar automáticamente ya digo que suele ser costumbre el que definamos un paquete base como este dónde colocar la clase notaba con arroba spring boot y a partir de ahí que coloquemos los sus paquetes necesarios hasta aquí la estructura de nuestro código vamos a pasar a hablar en el siguiente vídeo de la autoconfiguracion y esta anotación que configura automáticamente nuestro proyecto que es spring boot application
...
Hola a todos vamos a terminar este bloque sobre spring boot hablando de esa anotación mágica de Spring boot application para configurar últimamente la moda está en hacerlo a través de una mezcla de anotaciones y clases que estén anotadas con arroba configuration aplicaríamos la anotaciones sobre todo de estereotipo de componentes como por ejemplo@component services and obesity alguna serie de notas tienes más y con la clase anotadas con arroba configuration y métodos anotados con arroba bean dentro bueno pues podríamos hacer la configuración todo ello lo explica vamos en el curso de Spring core vale en el apartado dejaba confi también podemos usar property sabemos que se nos ha generado un fichero llamado application property dónde podemos poner algún elemento de configuración uno de los más usuales suele ser el cambio de Puerto el puerto 8080 no es que esté sobrevalorado sino que está sobre utilizado no y entonces en ocasiones no interesará que nosotros podamos cambiar de puerto eso lo podríamos hacer directamente en nuestro application properties vale y aquí podríamos cambiar el puerto por ejemplo al 9000 de manera que no tenemos que buscar ningún fichero extraño de configuración para cambiar nada sino que solamente con esta property si nosotros relanzamos nuestro proyecto y miramos la consola podemos comprobar como se ha lanzado la aplicación en el puerto 9000 en lugar de 80 francamente esto es cómodo todo esto queda orquestado gracias a la autoconfiguracion que se obtiene a través de la anotacion spring boot application es una notación derivada de otra del uso de otras como por ejemplo y neighbor autoconfiguration si os acordáis del capítulo de convención sobre Configuración decíamos que bueno el frigorífico a tomar una serie de decisiones de configuración por nosotros y qué bueno está en la íbamos a poder mudar de alguna manera sencilla y que además estás dependencia el perdón esta configuración dependía elementos que se encontraba porque no era con codigo autogenerado por lo cual si nosotros añadimos algunas dependencias a nuestro classpass por ejemplo la dependencia de H2 era ejemplo que poníamos en la Convención sobre Configuración y tenemos la anotación de Spring boot application bueno pues no encontraremos cómo se hace la autoconfiguracion de esa base de datos H2 que si no encuentra un bean DataSource que diga lo contrario o no se explícita lo contrario en el fichero de Property autoconfigurar y a la base de datos en memoria si pensamos que spring boot no configura y hace cosas por nosotros podríamos lanzar la dependencia perdón en la meta de Spring boot ROM desde meiben con el flag menos menos the book para ver todo lo que realmente se está cociendo por detrás no lo vamos a saber entender bien pero si es verdad que vamos a ver que salen muchísimas cosas cómo podéis comprobar antes de que se lance la aplicación en donde ya de por sí se están configurando un montón de elementos como por ejemplo una serie de filtros hablaremos de filtros en el entorno Java Web más adelante como por ejemplo donde van a estar localizada en las plantillas cosa que nosotros no hemos tenido que decir no invita a que añadamos plantillas de Zayn live algunos elementos más vale como las propiedades por defecto y el perfil que es el perfil default y si subimos podemos ver cómo ha hecho por nosotros una tarea enorme


Dónde se han incluido mogollón de librería que han sido configuradas con una serie de propiedades y que han quedado totalmente configurada para que nosotros podamos utilizar pues dicho elemento en nuestra aplicación de Spring boot y a Diego con una serie de propiedades normalmente que podemos encontrar por aquí que digo no vamos a comprenderlo todo pero si es verdad que vemos la cantidad ingente de tarea que spring boot hace por nosotros porque todas estas clases que se va encontrando el solo la paz las va configurando te queremos en alguna ocasión suele ser poco habitual pero puede ser que nos lo encontremos si en alguna ocasión queremos que spring boot no configure automáticamente alguna alguna clase porque nosotros queremos dar una configuración explícita que se sale del circuito del uso de streamwood podríamos hacerlo mediante la anotación excludes vale con la anotación en able autoconfiguration excludes sobre una clase anotada con arroba configura y son podríamos escribir pues que una determinada clase o incluso un determinado para que te quedará excluido de la de la autoconfiguracion cuando bueno se ponga autoejecutar perdón cuando se ponga configurar configurar a todos los elementos menos este que ha quedado totalmente fluido te digo que también lo podemos hacer mediante una lista de clase o también mediante la propiedad spring autoconfigure Esplugues vale en el fichero de Property y no teníamos que añadir ninguna anotación sino que lo podríamos hacer directamente de una forma textual y además ya sabes que los ficheros de properties pues no los podemos asociar a diferentes perfiles y podríamos tener una exclusión que funcionará en producción pero que no funcionara en el momento de desarrollo porque así no interesará hasta aquí el bloque de Spring boot en el siguiente en la siguiente elección del siguiente vídeo comenzamos con el bloque de Spring web MVC
