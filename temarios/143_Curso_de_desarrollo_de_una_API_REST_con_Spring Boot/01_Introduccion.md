...
Como elementos más avanzados tenemos también que Sprint ofrece el soporte para servicio red a través de Spring data rest qué es algo así como mezclar spring data con la generación rápida de una pirre y que veremos cómo es capaz de transformar rápidamente un repositorio de Spring data en toda una API con toda su controlador de ha montado y sin que tengamos que poner casi ni una línea de código adicional tenemos también el proyecto spring frito a es impronunciable esta palabra vale son las siglas de hipermedia hace en Jaén of the application state el striper media como el motor del estado de la aplicación que nos permite crear fácilmente este proyecto en la aplicación siguiendo este principio y además aprovechando el uso de cal de un lenguaje específico de hipertexto y qué bueno planteando la facilidad que existe a día de hoy en cualquier web de pinchando enlaces pásate un recurso otro que aplicarlo mismo nuestra API o también spring rest Docs vale que nos permite documentar un servicio red combinando sprint en visite o así Dr no todo este sería el soporte que no frece estén a la hora de crear nuestra nuestra API REST y que en las próximas lecciones pondremos en práctica
