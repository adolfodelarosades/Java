...
Vamos a seguir mejorando nuestra aplicación y para ello vamos a volver a recordar y vamos a ver cómo funciona en algunas clases y anotación ya hemos utilizado la de mapeo de método que más bean post mapping with Martin DB te mate también hemos utilizado pathvariable está no servía para desde un tramo de la ruta sacar un determinado valor que vienen tres llaves que inyectarlo viene realmente entre llaves la representación en una especie de plantilla si vienes por ella pero sacarlo desde allí e inyectarlo en un valor que en un argumento del metro del controlador y request body que nos vale para inyectar el cuerpo de una petición dentro de un objeto que se pasa como argumento método descontrolada que necesitamos las necesitamos ya responseentity la nombramos hace tiempo la necesitamos ya tres consentiti nos permite manejar de una forma más conveniente la respuesta que le enviamos al cliente y hemos visto antes que le daba de HTTP entity lo único que añade song status code código de respuesta vale y la posibilidad de enviar también cuerpo y de poder enviar respuesta 50 incluso tiene un Builder rápido que nos permitirá crear está este tipo de objeto no vamos a modificar la funcionalidad del ejemplo anterior añadiendo alguno elemento devolveremos en qeep mapping 200 si localizamos el recurso sino devolveremos un 404 Not Found que suele ser el código de respuesta plástico en postmapping en lugar de devolver 201 devolver perdón 200 devolveremos 201 vale que es el de creación de un nuevo recurso en put mapping devolveremos 200 okay si lo localizamos y modificamos sino de ciento perdón si no devolvemos 404 y en el delete mapping nos vamos a hacer en lugar de devolver el producto que eso realmente solamente serviría y vamos a hacer una especie de último preview de lo que acabamos de borrar que ha borrado si no pues podemos enviar un 204 no content con el cuerpo vacío tenemos que pensar siempre que lo que vayamos enviando como respuesta será siempre teniendo en mente que en este caso estamos programando para otros programadores que puede que sean de nuestro propio equipo o no estamos desarrollando una pirre para que gente que va a desarrollar aplicaciones cliente ya sea en angular en Android en iOS o en otro tipo de tecnología vayan a utilizar nuestra API REST no bueno pues teniendo esto en mente vamos a hacer las modificaciones sobre nuestro código nos venimos por aquí vale sobre que no es más que el de antes voy a hacer una nueva copia para que tengamos otro el código lo más organizado posible podemos ir viéndolo poquito a poco vamos a cambiar esto íbamos a ir haciendo una serie de modificaciones nosotros antes hemos devuelto directamente la el Finol del repositorio de productos lo que vamos a hacer que lo vamos a guardar en una lista llamar por ejemplo resulta y lo que vamos a hacer que si la lista está vacía lo que vamos a tratar es devolver una respuesta y si la lista tiene elementos devolvemos otra vale si la lista tiene elementos podríamos devolver directamente la lista vale pero si la lista no tiene elementos vamos a querer devolver una instancia de responseentity vale en la que vemos que tiene una serie de métodos como por ejemplo noto que directamente nos construyen responseentity con código 404 vale y que faltaría para construirlo llamar a Pili que pasa que en este caso sea que queríamos devolver un responseentity y aquí un resumen tenemos una un poco de discordia no lo podemos hacer refactorizando este método para la firma para que devuelva un responseentity en este caso de algo en este caso será con el cuerpo vacío o con el resultado vale con un okay pasándole el resultado como cuerpo podemos ver que ahora nuestro APIs bueno para tener un comportamiento distinto si tuviéramos productos o no lo estuvieras vale podemos comprobar que por ejemplo sigue funcionando en el buen sentido no el 404 lo haremos la comprobación con un producto que es más cómodo si no tendríamos que borrar todo lo que tengamos en la base de datos no si hacemos aquí una petición GET y no nada vale podemos ver como ahora no está devolviendo este 200 vale si si limpiamos la base de datos o comentábamos el data.sql Vale podríamos ver cómo nos devolvería ninguna vamos a hacer los cambios y entonces comprobamos aquí vale a la hora de buscar uno lo que hacíamos era esto no puedo porque vamos a guardar un producto Red Bull Valencia y ahora lo que hacemos sed que si result = NULL lo que hacemos es devolver igual que arriba response entity no content film y en otro caso devolvemos un response entity okay con resultado y tendríamos que cambiar esto vale aún response entity vale vamos a comprobar que funciona y ahora hacemos la misma de antes no sigue respondiendo si hacemos la petición GET de un producto nada más ejemplo del producto 1 pues me volvería el producto humo y hacemos la petición del producto 31 vemos como ahora expuesto no content es no perdón rápidamente cuenta por ahí código de respuesta no es 204 404 hacemos vemos cómo nos devuelve un código 404 qué significa que no ha encontrado el recurso es bastante más conveniente aquellos programadores que tú que utilicen nuestra API y que hagan una petición de búsqueda si reciben un 404 como respuesta sabrán que tendrán que mostrarle a usuarios bueno pues que no se ha encontrado el recurso o tendrán que hacer otro tipo de otro tipo de mecanismo nos vamos a la petición de post mapping vale tenemos aquí varias alternativa podemos recoger el producto en un nuevo producto y tendríamos la posibilidad existe el método created el problema están los siguientes para no tener que vale lo podíamos hacer así aquí pasar una URI lo normal red nos invita que cuando además de cuando se haga una petición POST para crear un recurso además de devolver un 201 se devuelva la URI dónde podemos consultar este ese producto como menos no podríamos hacer aquí a través de una base hardcodeada de la URL o utilizar unas clases que permiten manejar Lauri está en una parte incluso desde un servlet o desde una petición y construirla convenientemente como no nos vamos a parar a esa parte ahora para aprender también que podemos hacerlo podemos crear una nueva petición donde el código de estado lo pasemos o bien con un número o a través de las constantes de una enumeración qué es esta y podría ser HTTP status created vale y devolveremos ahí el cuerpo ya no es un volvemos un responseentity en este caso podría ser de producto vale porque estamos devolviendo podríamos recargar ahora real está el producto jamón de bellota ya se encuentra y de hecho si buscamos el producto perdón vemos como aquí estaré volviendo los 101 create vale se ha creado un nuevo recurso tanto y así que hace aquí devolvía 404 y ahora ya devuelven 200 porque ya está encontrando el producto 31 vemos que estudiaba respondiendo correctamente si a la hora tampoco lo vamos a programar ahora mismo pero si a la hora de crear el nuevo producto hubiera algún tipo de fallo por ejemplo que el nombre que no lo hemos marcado como obligatorio el mediante mecanismo de validación pero fuese obligatorio lo comprobaremos aquí no estuviera podríamos devolver a lo mejor un código 400 de peticiones no la petición de put mapping podemos cambiar el código para que en lugar de hacerse de esta manera se hiciera de una manera un poco más conveniente sabemos que bueno por lo pronto tendremos que volver a un responso sabemos que el producto repositorio tiene un método finebuy y ve al cuál le pasamos el Lidl y que devuelve una opcional pues opcional tiene un método fantástico que nos va a permitir transformar el objeto en otro objeto y si no vale este lo vamos a hacer ahora aquí ya veremos todo lo que vamos a incluir y si no vale esto sería y sino que nos devuelva en lugar de ese objeto vale vale que ya veremos la quesera bueno por lo pronto esto será lo que se devuelva y como tenemos aquí este Maps vale ahora se caerá correctamente cuando ahora se perderá esto esto viene porque es un producto bueno ahora lo podremos ir solucionando lo que devolvamos aquí dentro será el producto editado y aquí dentro ya no podemos viendo si no ha encontrado el producto lo que hacemos devolver pon mane.com y aquí lo que vamos a ir haciendo eh el nombre del producto será el que hemos pasado y lo que tenemos es que devolverlo cómo podemos comprobar que no faltaba que era devolver en retorno a Monte devolviendo en el Maps pon responseentity del producto ya almacenado porque lo estamos almacenando aquí con código ok 200 OK y el código y el producto modificado en lo que vamos buscando y si no lo que se devuelve aquí sería un noto porque no lo hemos encontrado el comportamiento que andábamos buscando no recordamos aquí nuestro jamón de bellota lo transformamos a ibérico de bellota y vemos que sucede lo mismo es lo que quisiéramos sería transformar el producto 33 que no existe sino te volvería de nuevo o 404 qué es lo que hablamos por último a la hora de modificar como hemos dicho que ya no vamos a querer devolver lo podríamos hacer vale nos podríamos casi que quitar todo este código vamos delete eBay aire repasamos el líder y aquí si podemos hacer una devolución hemos creado el 31 y ahora vamos a borrarlo y nos devolvería vacío pero no devuelve el código correcto que es 204 y así hemos completado nuestra API para que los códigos de retorno que vamos teniendo se vayan acercando a lo que debería
