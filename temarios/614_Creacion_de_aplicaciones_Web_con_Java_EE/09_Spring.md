...
, vamos a ver cómo acceder a base de datos con Spring y más concretamente utilizando spring JDBC ya que hay un objeto dentro del módulo explico otra vez y conocido como JDBC template que simplemente pues conociendo las instrucciones SQL quitando esas instrucciones para lanzar a la base datos pues un poco más vamos a tener que hacer y todo el proceso que se hace normalmente jlcd crear conexiones el statement cetera etcétera está encapsulado dentro de los métodos que nos va a proporcionar el JVC temple por lo tanto las tareas de acceso a datos desde la aplicación Spring Summer de módulos van a ser muchísimo más sencillas que con el J&R televisión lo primero que dependencias deberíamos incluir en nuestro proyecto para utilizar spring JDBC liamos liamos dentro del pon XML la dependencia al módulo spring JDBC con la versión que estemos manejando de Spring y resto de los módulos y por supuesto pues el conector de MySQL o de la base datos con la que está trabajando driver driver para trabajar con MySQL utilizando en cada momento y para ello se basa en el uso de un datáfono por lo tanto a la hora de configurar JDBC template para poder utilizarlo después en la aplicación pues por un lado te hemos registrar un objeto gato azul crear un objeto azul en definitiva dentro de la clase de configuración disponer de un método que va bien que nos devuelva un driver manager data azul que es la clase de Spring que implementa la interfaz a luz ya sabemos cómo funciona nota sol en este caso pues en el centro que estás viendo lo crearía directamente spring el propiedad azul aunque también sabemos que los datos son son objetos que viven en el servidor de aplicaciones entonces no necesitaríamos crearlo desde aquí si no utilizarlo llamarlo directamente desde nuestra clase de configuración ya veremos también cuando veamos la aplicación de ejemplo pues veremos las dos casos utilizando este dat sur creado por el propio spring que lo incorpora la hermana llegadas hoy de una clase para que no tanta Sur y crearlos desde código o bien utilizando el del servidor de aplicación en el caso de que creemos este objeto driver manager data sol pues habrá que proporcionarle la diberclase la cadena de conexión a la base de datos el usuario y la contraseña qué hacemos con ese objeto pues luego utilizarlo para construir el JDBC template otro método J Balvin te devuelva JDBC template será encargado de crear este objeto un objeto objeto definido en la línea por lo tanto pues en el momento que nosotros ya íbamos struction sabemos que aquí en esta variable estará inyectado el otro objeto azul que spring habrá que qué métodos nos proporciona JDBC template para poder operar con la base de datos cuando nosotros necesitemos lo hace internamente que se me produce si la instrucción SQL esta parametrizada que sea habitual pues los valores de los parámetros son los que se pasan como segundo tercer argumento jejeje así de simple que no devuelve el método de número de filas afectadas es muy sencillo se trata una consulta de selección de registros pues entonces utilizaremos el método point implica un poquito más de trabajo como ya veremos cuando hagamos la aplicación de ejemplo pero también es muy simple decir método que utilizamos tres parámetros el primero instrucción serie secuela qué tipos voy a dejar al final pues la lista de valores de los argumentos en caso de parametriza y bueno qué es lo que nos devuelve nos devuelve la lista de objetos los objetos de decirte a mapeados las pilas a los objetos que estamos con los que estamos trabajando objetos curso jefalun no he cambiado lo que sea para hacer ese mapeo de filas la base datos objetos pues tenemos que proporcionar en este segundo parámetro una implementación de la interfaz Roomba te como ya veremos cuando hagamos ejemplo interfaz Roomba tiene un único método en el que se recibe como parámetro un reset y devuelve un objeto que estamos trabajando en el interior de ese método tenemos que implementar las instrucciones que transformen la fila actualmente apuntando el resultat en una mujer llamando automáticamente spring con cada fila con la que vaya pasando el resuelto normalmente aquí utilizaremos una expresión Landa para proporcionar la implementación de centros para roja pero bueno si estoy solo ya veremos un ejemplo concreto en definitiva es métodos muy sencillos de manejar último comentar que bueno pues ahora que ya vamos a incorporar el acceso datos en nuestra aplicación web es habitual y conveniente realizar una separación en esa modelo en esa lógica de negocio en tus gafas decir lo que sería la capa de servicio y la capa repository repositorio sería pues la que se encarga principalmente de todas las operaciones de acceso o sea no vamos a mezclar las estaciones nosotros con nuestra lógica de negocio lo vamos a separar o qué hacen aplicaciones sencillas resulta un poco digamos por ser redundante hacer ciertas cosas pero es muy conveniente que nos vayamos acostumbrando a hacer eso para con las aplicaciones grandes pues vamos a ver enormemente los beneficios que estos nos proporcionan como digo la capa repositorio pues estará formada por una interfaz igualmente o sea como todo en Spring una interfaz que define los métodos y una clase que no simplemente métodos básicos de operar contra la base datos pues hacer las intenciones las recuperaciones actualizaciones eliminaciones todo muy básico donde utilizaremos pues todo lo que ya es lado método de la lógica

, que va a llamar el controlador o nos enfrentaríamos en una capa independiente en la llamada capa de servicio que igualmente tendrías princesa de negocio interfaz dónde se definen los métodos y la clase que los implementa aquí lo queríamos es como está viendo al inyectar la capa repositorio para pues lógicamente hacer la llamada ya lo veremos muy claro en el sitio que vamos a realizar pero ya digo esas situaciones de Spring JDBC quedan aquí la capa repositorio no en las 20 instrucciones qué quiere decir que si mañana cambiamos de repositorio pero utilizar pues estoy JDBC utilizamos serán sustituidos por la misma firma bastante conveniente que vamos a tener que ese monos que la clase de la mente se suele notar mientras que la capa de servicio
