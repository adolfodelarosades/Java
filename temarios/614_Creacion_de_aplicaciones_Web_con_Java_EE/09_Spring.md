...
metro una vez que ya tenemos implementado el controlador lógicamente el modelo ahora de momento lo dejamos un poquito de lado al fin al cabo el modelo son clases normales y corrientes como las que ya hemos utilizado con servlet y JSP pero bueno ya veremos más detenidamente qué peculiaridades había que tener en cuenta usando spring pero centrándonos en el controlador pues una vez que están implementados los métodos controladores de acción en las distintas clases controles que tengamos pues por un lado en el archivo web.xml como hemos indicado anteriormente debemos registrar el DispatcherServlet qué es el Front controller necesita unipan llamado Condis contextconfiglocation cuyo Malú es la dirección de los archivos de configuración de Spring adicionales aparte del Web XML en este caso porque estamos suponiendo que tenemos en el mismo y un Springfield para el modelo podría hacer este para el modelo y la contraria digo que los nombres son indiferentes los verás qué buena comúnmente pues para la parte de controlado y le vamos a llamar así me voy te conocí y la configuración del modelo Springfield en el primer ejemplo que vamos a hacer a continuación pues no vamos a utilizar modelo entonces solamente estaría esta dirección la del el controlador en MVC config que ahora vamos a ver cómo sería por otro lado a quién se le matin pues a través de Google épater le indicamos la barra qué quiere decir que cualquier petición cualquier petición te llega esta aplicación web tiene que pasar por el despacho

Tiene que pasar por el dispatcherservlet' se va a encargar de iniciar spring de resolver las anotaciones que le hemos indicado a los distintas clases para crear las instancias resuelve la inyección de dependencia nos quedaría pues eso los archivos para controlador poblado tendríamos una etiqueta station driver qué le dice el indica spring que permita el uso de anotaciones para configuración las anotaciones Aruba controller busca todas las clases que tengan esa anotación especial para distancia de momento que spring se inicie por otro lado tenemos este objeto anotado con la nota con la etiqueta guapa está registrado con la etiqueta PIN que es el que se encarga de resolver las vistas ya hemos dicho anteriormente que los controladores de acción devuelven un string en una cadena de caracteres que solo lo interpreta como la dirección de la página eso es lo que le indica este objeto que estamos registrando aquí cuando registramos un objeto propio de Spring consiste internal absolvió resuelven el XML tenemos que indicarlo con la nota con la etiqueta identificadora en Clash pues a través de la propia prefix dónde se encuentran las páginas para buscar las decir cuando en el metro controlacion nosotros devolvemos el retour una cadena buscar en esta dirección una página que tenga ese nombre y la extensión indicada en suffix está abierto para utilizar otras tecnologías para implementación de vista en este caso es de internal zombie resuelven lo que hace es indicarle que tiene que buscar en esta dirección no sean el raíz de la aplicación de la aplicación pues una página con ese nombre por otro lado finalmente pues ahí peticiones que no necesitan ser gestionadas por un control acción sino que simplemente llevarle al usuario una determinada página como le invitamos a despachos al DispatcherServlet que haga esa tarea pues a través de los controladores para navegación estática son etiquetas view controller en el que le dicen cuando llegue la dirección sería lo que viene después del corte root de la de la dirección base de la aplicación pues si viene una petición con la dirección alta entonces te tenemos que irnos a la vista tus datos que he puesto para resolverte dónde está esa página y qué estación tiene utilizaría print linterna resolvió resulten realmente son controladores creación de acción que no hay que empezar que ya están hechos simplemente para navegación estática estática en estas direcciones que le lleve directamente a estas páginas JSP lo vamos a ver ahora en funcionamiento en el primer ejercicio que vamos a desarrollar en 7 días
