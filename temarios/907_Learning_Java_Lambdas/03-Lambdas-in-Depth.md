...
Echemos un vistazo al código de bytes producido para ver algunos ejemplos.

Vamos a ver el código de bytes de cuatro bloques distintos de funcionalidad según el ejemplo que vimos en la sección lambdas vs closures.

Exploraremos:

1. Una clase anónima simple.
2. Una clase anónima que se cierra sobre alguna variable (un cierre de estilo antiguo).
3. Una lambda sin argumentos.
4. Una lambda con argumentos.
5. Un cierre lambda sobre alguna variable (un closure de nuevo estilo).

El código de bytes de ejemplo se generó utilizando la herramienta **javap** de línea de comando. En esta sección solo se muestran listados parciales de códigos de bytes; para obtener listados completos de fuentes y códigos de bytes, consulte Appendix A. Además, los nombres de clases completos se han acortado para que quepan mejor en la página.
