...
Comparador<Apple> porPeso = nuevo Comparador<Apple>() {
...
(parámetros) -> { declaraciones; } 
    Lista<Apple> resultado = nueva ArrayList<>(); 
    para (Apple manzana: inventario) { 
        if ( 
            resultado.add(manzana); 
        } 
    } 
    devolver resultado; 
}
...
Una expresión booleana	(Lista<Cadena> lista) -> list.isEmpty()
Creando objetos	() -> nueva Apple(10)
Consumir de un objeto	(Apple a) -> {
...
Lista<Apple>manzanas verdes =
        filtrar(inventario, (Apple a) -> GREEN.equals(a.getColor()) );
...
Predicado de interfaz pública<T> {
    prueba booleana (T t);
}
...
Comparador de interfaz pública<T> {                            1
    int comparar(T o1, T o2);
}
interfaz pública ejecutable {                                 2
    ejecución vacía();
}
interfaz pública ActionListener extiende EventListener {     3
    acción nula realizada (ActionEvent e);
}
interfaz pública invocable<V> {                              4
    V call() arroja una excepción;
}
interfaz pública Acción Privilegiada<T> {                      5
    T ejecutar();
}
...
sumador de interfaz pública {
    int agregar(int a, int b);
}
interfaz pública SmartAdder extiende Adder {
    int add(doble a, doble b);
}
interfaz pública Nada {
}
Respuesta:
...
Ejecutable r1 = () -> System.out.println("Hola mundo 1");         1 
Ejecutable r2 = nuevo Ejecutable() {                                    2
    ejecución pública vacía() {
        System.out.println("Hola mundo 2");
    }
};
proceso público vacío estático (Runnable r) {
    r.run();
}
proceso(r1);                                                     3 
proceso (r2);                                                     4 
proceso(() -> System.out.println("Hola mundo 3"));              5
...
proceso de anulación pública (Runnable r) {
    r.run();
}
proceso(() -> System.out.println("¡¡Esto es increíble!!"));
...
proceso(() -> System.out.println("Esto es increíble"));
Después de todo, System.out.printlnregresa , void¡así que claramente esto no es una expresión! ¿Por qué no tenemos que encerrar el cuerpo con llaves como ésta?
...
ejecutar(() -> {});
ejecución pública vacía (ejecutable r) {
  r.run();
}
publicable invocable<String> fetch() {
  return () -> "Ejemplo complicado ;-)";
}
Predicado<Apple> p = (Apple a) -> a.getWeight();
Respuesta:
...
public String ProcessFile() lanza IOException {
...
@Interfaz funcional
interfaz pública BufferedReaderProcessor {
    El proceso de cadena (BufferedReader b) arroja IOException;
}
Ahora puede utilizar esta interfaz como argumento de su nuevo processFilemétodo:

archivo de proceso de cadena pública (BufferedReaderProcessor p) lanza IOException {
   ...
}
...
archivo de proceso de cadena pública (BufferedReaderProcessor p) lanza IOException {
    intentar (BufferedReader br =
                   nuevo BufferedReader (nuevo FileReader ("datos.txt"))) {
        devolver p.proceso(br);                                           1
    }
}
...
filtro público <T> Lista<T> (Lista<T> lista, Predicado<T> p) {
    Lista<T> resultados = nueva ArrayList<>();
    para(T t: lista) {
        si(p.prueba(t)) {
            resultados.add(t);
        }
...
Predicado<Cadena> nonEmptyStringPredicate = (Cadena s) -> !s.isEmpty(); 
Lista<Cadena> noEmpty = filtro(listOfStrings, nonEmptyStringPredicate );
...
@Interfaz funcional
interfaz pública Consumidor<T> {
    aceptar vacío(T t);
}
public <T> void forEach(Lista<T> lista, Consumidor<T> c) {
    para(T t: lista) {
        c.aceptar(t);
    }
}
para cada(
        ...
        (Entero i) -> System.out.println(i)          1 
       );
...
@Interfaz funcional
Función de interfaz pública<T, R> {
    R aplicar(T t);
}
público <T, R> Lista<R> mapa(Lista<T> lista, Función<T, R> f) {
    Lista<R> resultado = nueva ArrayList<>();
    para(T t: lista) {
        resultado.add(f.apply(t));
    }
    resultado de devolución;
}
// [7, 2, 6]
Lista<Entero> l = mapa(
                       Arrays.asList("lambdas", "en", "acción"),
                       (Cadena s) -> s.length()                      1 
               );
...
Lista<Integer> lista = nueva ArrayList<>();
para (int i = 300; i < 400; i++){
    lista.add(i);
}
...
interfaz pública IntPredicate {
    prueba booleana (int t);
}
IntPredicate números pares = (int i) -> i % 2 == 0;
números pares.prueba(1000);                                        1
Predicado<Entero> Números impares = (Entero i) -> i % 2 != 0;
números impares.prueba(1000);                                         2
...
Una expresión booleana	(Lista<Cadena> lista) -> list.isEmpty()	Predicado<Lista<Cadena>>
Creando objetos	() -> nueva Apple(10)	Proveedor<Apple>
Consumir de un objeto	(Apple a) ->
System.out.println(a.getWeight())	Consumidor<Apple>
Seleccionar/extraer de un objeto	(Cadena s) -> s.length()	Función<Cadena,
Entero> o
ToIntFunction<Cadena>
Combina dos valores	(int a, int b) -> a * b	OperadorIntBinario
comparar dos objetos	(Apple a1, Apple a2) ->
a1.getWeight().compareTo(a2.getWeight
())	Comparador<Apple> o
...
Función<BufferedReader, Cadena> f =
  (Lector almacenado en búfer b) -> {
    intentar {
      devolver b.readLine();
    }
...
Lista <Apple> más pesada que 150 g =
        filtrar(inventario, (Apple manzana) -> apple.getWeight() > 150 );
...
Invocable<Entero> c = () -> 42;
Acción Privilegiada<Entero> p = () -> 42;
...
ToIntBiFunction<Apple, Apple> c2 =
  (Apple a1, Apple a2) -> a1.getWeight().compareTo(a2.getWeight());
BiFunción<Apple, Apple, Entero> c3 =
  (Apple a1, Apple a2) -> a1.getWeight().compareTo(a2.getWeight());
...
Lista<Cadena> listaDeCadenas = nueva ArrayList<>();
Lista<Integer> listOfIntegers = nueva ArrayList<>();
Regla especial de compatibilidad nula
...
// El predicado tiene un retorno booleano
Predicado<Cadena> p = (Cadena s) -> list.add(s);
// El consumidor tiene una devolución nula
Consumidor<String> b = (String s) -> list.add(s);
...
Objeto o = () -> { System.out.println("Ejemplo complicado"); };
Respuesta:
...
Ejecutable r = () -> { System.out.println("Ejemplo complicado"); };
...
Objeto o = (Ejecutable) () -> { System.out.println("Ejemplo complicado"); };
...
ejecución pública vacía (ejecutable ejecutable) {
    ejecutable.run();
}
ejecución pública nula (Acción<T> acción) {
    acción.actuar();
}
@Interfaz funcional
acción de interfaz {
...
Lista<Apple>manzanas verdes =
        filtro(inventario, manzana -> VERDE.equals(apple.getColor()));     1
...
int número de puerto = 1337;
Ejecutable r = () -> System.out.println(portNumber);
...
int número de puerto = 1337;
Ejecutable r = () -> System.out.println(portNumber);         1 
número de puerto = 31337;
...
inventario.sort(comparando( Apple::getWeight ));          1
...
Lista<Cadena> str = Arrays.asList("a","b","A","B");
...
ToIntFunction<Cadena> cadenaToInt =
               (Cadena s) -> Integer.parseInt(s);
BiPredicate<Lista<Cadena>, Cadena> contiene =
               (lista, elemento) -> lista.contiene(elemento);
Predicado<Cadena> comienzaConNúmero =
               (Cadena cadena) -> this .starts-WithNumber(cadena);
Respuestas:
...
Proveedor<Apple> c1 = Apple::nuevo;          1 
manzana a1 = c1.get();                      2
...
Proveedor<Apple> c1 = () -> nueva Apple();         1 
manzana a1 = c1.get();                            2
...
Función<Entero, Apple> c2 = Apple::new;        1 
manzana a2 = c2.apply(110);                        2
...
Función<Entero, Apple> c2 = (peso) -> nueva Apple(peso);     1 
manzana a2 = c2.apply(110);                                        2
...
En el siguiente código, cada elemento de a Listof Integers se pasa al constructor usando Appleun mapmétodo similar que definimos anteriormente, lo que da como resultado a Listof apples con varios pesos:

Lista<Integer> pesos = Arrays.asList(7, 3, 4, 10);
Lista<Apple> manzanas = mapa(pesos, Apple::new);                            1
Lista pública <Apple> mapa (Lista <Integer> lista, Función <Integer, Apple> f) {
    Lista<Apple> resultado = nueva ArrayList<>();
    para (Entero i: lista) {
        resultado.add(f.apply(i));
    }
    resultado de devolución;
}
...
BiFunción<Color, Entero, Manzana> c3 = Manzana::nueva;      1 
manzana a3 = c3.apply(VERDE, 110);                        2
...
BiFunción<Cadena, Entero, Manzana> c3 =
    (color, peso) -> nueva Apple(color, peso);        1 
manzana a3 = c3.apply(VERDE, 110);                        2
...
Mapa estático<Cadena, Función<Entero, Fruta>> mapa = nuevo HashMap<>();
estático {
    map.put("manzana", Apple::nuevo);
    map.put("naranja", Naranja::nuevo);
    // etc...
}
Fruta estática pública GiveMeFruit (fruta en cadena, peso entero) {
    devolver map.get(fruit.toLowerCase())                               1 
              .apply(peso);                                        2 
}
...
TriFunction<Entero, Entero, Entero, RGB> colorFactory = RGB::nuevo;
