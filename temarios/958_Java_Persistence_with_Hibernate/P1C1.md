...
La mejor manera de aprender Hibernate no es necesariamente lineal. Entendemos que es posible que desee probar Hibernate de inmediato. Si así es como desea proceder, salte al siguiente capítulo y configure un proyecto con el ejemplo "Hola mundo". Le recomendamos que regrese aquí en algún momento a medida que avanza en este libro; de esa manera, estará preparado y tendrá todos los conceptos básicos que necesita para el resto del material.
...
Figura 1.1.Un diagrama UML simple de las entidades UseryBillingDetails


En este diagrama, puedes ver que a Usertiene muchos BillingDetails. Puede navegar la relación entre las clases en ambas direcciones; esto significa que puede iterar a través de colecciones o llamar a métodos para llegar al "otro" lado de la relación. Las clases que representan estas entidades pueden ser extremadamente simples:

 Usuario de clase pública {
<entrar/>
    Nombre de usuario de cadena ;
    Dirección de cadena ;
    Establecer detalles de facturación;
<entrar/>
    // Métodos de acceso (getter/setter), métodos comerciales, etc.
}
<entrar/>
 detalles de facturación de clase pública {
<entrar/>
    Cuenta de cadena ;
    Nombre de banco de cadena ;
    usuario usuario ;
<entrar/>
    // Métodos de acceso (getter/setter), métodos comerciales, etc. 
}
...
crear tabla USUARIOS (
    NOMBRE DE USUARIO varchar ( 15 ) clave principal no nula ,
    DIRECCIÓN varchar ( 255 ) no nulo
);
<entrar/>
crear tabla BILLINGDETAILS (
    CUENTA varchar ( 15 ) clave principal no nula ,
    NOMBRE DEL BANCO varchar ( 255 ) no nulo ,
    NOMBRE DE USUARIO varchar ( 15 ) no nulo ,
    clave externa (NOMBRE DE USUARIO) referencias USUARIOS
);
La columna de clave foránea restringida USERNAMErepresenta BILLINGDETAILSla relación entre las dos entidades. Para este modelo de dominio simple, el desajuste objeto/relacional es apenas evidente; es sencillo escribir código JDBC para insertar, actualizar y eliminar información sobre usuarios y detalles de facturación.
...
El problema más obvio con la implementación actual es que ha diseñado una dirección como un Stringvalor simple. En la mayoría de los sistemas, es necesario almacenar la información de la calle, la ciudad, el estado, el país y el código postal por separado. Por supuesto, podría agregar estas propiedades directamente a la Userclase, pero debido a que es muy probable que otras clases en el sistema también contengan información de direcciones, tiene más sentido crear una Addressclase. La figura 1.2 muestra el modelo actualizado.

Figura 1.2.El Usertiene un Address.


¿ Debería agregar también una ADDRESStabla? No necesariamente; es común mantener la información de la dirección en la USERStabla, en columnas individuales. Es probable que este diseño funcione mejor, porque no se necesita una combinación de tabla si desea recuperar el usuario y la dirección en una sola consulta. La mejor solución puede ser crear un nuevo tipo de datos SQL para representar direcciones y agregar una sola columna de ese nuevo tipo en la USERStabla en lugar de varias columnas nuevas.
...
Volvamos al ejemplo. Agregar un nuevo tipo de datos al catálogo de la base de datos, para almacenar Addressinstancias de Java en una sola columna, parece el mejor enfoque:

crear tabla USUARIOS (
    NOMBRE DE USUARIO varchar ( 15 ) clave principal no nula ,
    DIRECCIÓN dirección no nula 
);
Un nuevo Addresstipo (clase) en Java y un nuevo ADDRESStipo de datos SQL deberían garantizar la interoperabilidad. Pero encontrará varios problemas si verifica la compatibilidad con los tipos de datos definidos por el usuario (UDT) en los sistemas de administración de bases de datos SQL actuales.
...
Por estas y otras razones, el uso de UDT o tipos de Java en una base de datos SQL no es una práctica común en la industria en este momento, y es poco probable que encuentre un esquema heredado que haga un uso extensivo de UDT. Por lo tanto, no puede y no almacenará instancias de su nueva Addressclase en una sola columna nueva que tenga el mismo tipo de datos que la capa de Java.

La solución pragmática para este problema tiene varias columnas de tipos de SQL integrados definidos por el proveedor (como tipos de datos booleanos, numéricos y de cadena). Por lo general, define la USERStabla de la siguiente manera:

crear tabla USUARIOS (
    NOMBRE DE USUARIO varchar ( 15 ) clave principal no nula ,
    ADDRESS_STREET varchar ( 255 ) no es nulo ,
    ADDRESS_ZIPCODE varchar( 5 ) no nulo ,
    ADDRESS_CITY varchar ( 255 ) no nulo 
);
Las clases en el modelo de dominio de Java vienen en una gama de diferentes niveles de granularidad: desde clases de entidad de grano grueso como User, hasta clases de grano más fino como Address, hasta simples SwissZipCodeextensiones AbstractNumericZipCode(o cualquiera que sea el nivel de abstracción que desee). Por el contrario, solo dos niveles de granularidad de tipo son visibles en la base de datos SQL: tipos de relación creados por usted, como USERSy BILLINGDETAILS, y tipos de datos integrados como VARCHAR, BIGINTo TIMESTAMP.
...
La Userclase tiene una asociación con la BillingDetailssuperclase. Esta es una asociación polimórfica . En tiempo de ejecución, una Userinstancia puede hacer referencia a una instancia de cualquiera de las subclases de BillingDetails. De manera similar, desea poder escribir consultas polimórficas que se refieran a la BillingDetailsclase y que la consulta devuelva instancias de sus subclases.
...
El siguiente aspecto del problema del desajuste entre el objeto y la relación es la cuestión de la identidad del objeto . Probablemente haya notado que el ejemplo se define USERNAMEcomo la clave principal de la USERStabla. ¿Fue una buena elección? ¿Cómo manejas objetos idénticos en Java?
...
Igualdad de instancia, según lo determinado por la implementación del equals()método (también llamada igualdad por valor )
Por otro lado, la identidad de una fila de la base de datos se expresa como una comparación de los valores de la clave principal. Como verá en la sección 10.1.2 , ni equals()tampoco ==es siempre equivalente a una comparación de valores de clave principal. Es común que varias instancias no idénticas en Java representen simultáneamente la misma fila de la base de datos, por ejemplo, en subprocesos de aplicaciones que se ejecutan simultáneamente. Además, algunas dificultades sutiles están involucradas en la implementación equals()correcta para una clase persistente y en la comprensión de cuándo esto podría ser necesario.

Usemos un ejemplo para discutir otro problema relacionado con la identidad de la base de datos. En la definición de la tabla para USERS, USERNAMEes la clave principal. Desafortunadamente, esta decisión dificulta cambiar el nombre de un usuario; necesita actualizar no solo la fila en USERS, sino también los valores de clave externa en (muchas) filas de BILLINGDETAILS. Para resolver este problema, más adelante en este libro le recomendamos que utilice claves sustitutas siempre que no pueda encontrar una buena clave natural. También discutimos lo que hace una buena clave primaria. Una columna de clave sustituta es una columna de clave principal sin significado para el usuario de la aplicación; en otras palabras, una clave que no se presenta al usuario de la aplicación. Su única finalidad es la identificación de datos dentro de la aplicación.

Por ejemplo, puede cambiar las definiciones de su tabla para que se vean así:

crear tabla USUARIOS (
    ID bigint clave primaria no nula ,
    NOMBRE DE USUARIO varchar ( 15 ) no nulo único,
    ...
...
crear tabla BILLINGDETAILS (
    ID bigint clave primaria no nula ,
    CUENTA varchar ( 15 ) no nulo ,
    NOMBRE DEL BANCO varchar ( 255 ) no nulo ,
    USER_ID bigint no nulo ,
    referencias de clave externa (USER_ID) USUARIOS
);
Las IDcolumnas contienen valores generados por el sistema. Estas columnas se introdujeron únicamente en beneficio del modelo de datos, entonces, ¿cómo deberían representarse (si es que lo hacen) en el modelo de dominio de Java? Discutimos esta pregunta en la sección 4.2 y encontramos una solución con ORM.
...
En su modelo de dominio, las asociaciones representan las relaciones entre entidades. Las clases User, Addressy BillingDetailsestán todas asociadas; pero a diferencia de Address, BillingDetailsse sostiene por sí mismo. BillingDetailslas instancias se almacenan en su propia tabla. El mapeo de asociaciones y la gestión de asociaciones de entidades son conceptos centrales en cualquier solución de persistencia de objetos.

Los lenguajes orientados a objetos representan asociaciones usando referencias a objetos ; pero en el mundo relacional, una columna restringida por clave externa representa una asociación, con copias de valores clave. La restricción es una regla que garantiza la integridad de la asociación. Hay diferencias sustanciales entre los dos mecanismos.
...
Pero la declaración de clave externa sobre la BILLINGDETAILSmesa es una asociación de muchos a uno : cada cuenta bancaria está vinculada a un usuario en particular. Cada usuario puede tener varias cuentas bancarias vinculadas.

Si desea representar una asociación de muchos a muchos en una base de datos SQL, debe introducir una nueva tabla, generalmente llamada tabla de enlace . En la mayoría de los casos, esta tabla no aparece en ninguna parte del modelo de dominio. Para este ejemplo, si considera que la relación entre el usuario y la información de facturación es de muchos a muchos , defina la tabla de vínculos de la siguiente manera:

crear tabla USER_BILLINGDETAILS (
    USER_ID bigint,
    BILLINGDETAILS_ID bigint,
    clave principal (USER_ID, BILLINGDETAILS_ID),
    la clave externa (USER_ID) hace referencia a USUARIOS,
    referencias de clave externa (BILLINGDETAILS_ID) BILLINGDETAILS
);
Ya no necesita la USER_IDcolumna de clave externa y la restricción en la BILLINGDETAILStabla; esta tabla adicional ahora gestiona los enlaces entre las dos entidades. Discutimos los mapeos de asociaciones y colecciones en detalle en el capítulo 7 .
