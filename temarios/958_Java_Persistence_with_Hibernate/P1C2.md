...
4. **Hibernate** puede escanear su classpath en busca de clases asignadas(mapped classes) y agregarlas automáticamente a su unidad de persistencia. Esta configuración deshabilita esa función.
...
6. El motor JPA debería descartar y volver a crear el esquema SQL en la base de datos automáticamente cuando se inicia. Esto es ideal para pruebas automatizadas, cuando desea trabajar con una base de datos limpia para cada ejecución de prueba.
...
Todas las declaraciones SQL ejecutadas por Hibernate pueden ser logged, una herramienta invaluable durante la optimización. Para ver el log SQL, en el archivo **`persistence.xml`**, establezca las propiedades **`hibernate.format_sql`** y **`hibernate.use_sql_comments`** a **`true`**. Esto hará que Hibernate formatee declaraciones SQL con comentarios de causalidad. Luego, en su logging configuration (que depende de la implementación de logging elegido), establezca las categorías **`org.hibernate.SQL`** y **`org.hibernate.type.descriptor.sql.BasicBinder`** el nivel de depuración(debug level) más fino. Luego verá todas las declaraciones SQL ejecutadas por Hibernate en su salida de registro(log output), incluidos los valores de los parámetros vinculados de las declaraciones preparadas.
